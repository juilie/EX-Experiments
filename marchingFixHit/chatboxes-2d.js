export class Chatboxes2D {
    ...
    setupMouseTracking() {
        // Track mouse in canvas coordinate space (account for pixelScale)
        window.addEventListener('mousemove', function(e) {
            this.mouseX = e.clientX / this.pixelScale;
            this.mouseY = e.clientY / this.pixelScale;
        }.bind(this));
    }
    ...
    drawChatboxes() {
        ...
        this.chatboxes2D = this.chatboxes2D.filter(chatbox => {
            ...
            // Hover to destroy: if mouse within bounds, remove and spawn damage text
            if (this.mouseX >= 0 && this.mouseY >= 0) {
                const withinX = this.mouseX >= chatbox.x && this.mouseX <= chatbox.x + chatbox.width;
                const withinY = this.mouseY >= chatbox.y && this.mouseY <= chatbox.y + chatbox.height + (20 / this.pixelScale);
                if (withinX && withinY) {
                    // 20% chance of CRITICAL, otherwise a 3-4 digit number
                    const isCritical = Math.random() < 0.2;
                    const value = isCritical ? 'CRITICAL' : String(900 + Math.floor(Math.random() * 1500));
                    this.spawnDamageText(chatbox.x + chatbox.width / 2, chatbox.y, isCritical, value);
                    return false; // remove this chatbox
                }
            }
        ...
        // Update and draw floating damage texts
        this.drawDamageTexts(currentTime);
    }
    ...
    spawnDamageText(centerX, topY, isCritical, value) {
        const text = value || (isCritical ? 'CRITICAL' : String(900 + Math.floor(Math.random() * 1500)));
        const lifetime = isCritical ? 900 : 700; // ms
        this.damageTexts.push({
            ...
            startTime: performance.now(),
            lifetime: lifetime,
            vy: isCritical ? -0.2 : -0.15, // canvas units per ms
            scale: isCritical ? 1.2 : 1.0
        });
    }

    drawDamageTexts(currentTime) {
        const fontFamily = this.customFontLoaded ? 'Epmarugo' : 'monospace';
        const baseFont = 28 / this.pixelScale;
        this.damageTexts = this.damageTexts.filter(dt => {
            const elapsed = currentTime - dt.startTime;
            const t = Math.min(1, elapsed / dt.lifetime);
            // Update position
            dt.y += dt.vy * (currentTime - dt.startTime) * 0.06; // scaled drift
            dt.startTime = currentTime; // reuse as previous frame timestamp
            // Alpha ease out
            const alpha = 1 - t;
            const fontSize = baseFont * (dt.isCritical ? 1.6 : 1.2) * (1 + 0.2 * (1 - t))
            this.overlayCtx.save();
            this.overlayCtx.globalAlpha = Math.max(0, alpha);
            // Shadow/outline
            this.overlayCtx.lineWidth = Math.max(1, Math.round(4 / this.pixelScale));
            this.overlayCtx.textAlign = 'center';
            this.overlayCtx.textBaseline = 'bottom';
            this.overlayCtx.font = `${dt.isCritical ? '900' : '700'} ${fontSize}px ${fontFamily}`;
            this.overlayCtx.strokeStyle = 'black';
            this.overlayCtx.fillStyle = dt.isCritical ? '#ffd166' : '#e63946';
            this.overlayCtx.strokeText(dt.text, dt.x, dt.y);
            this.overlayCtx.fillText(dt.text, dt.x, dt.y);
